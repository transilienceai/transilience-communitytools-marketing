...
[
  ...
  ...
    ...
    for attempt in range(max_retries):
        try:
            raw = generate_text(prompt=prompt, max_output_tokens=2000, api_key=api_key)
            if not raw:
                continue
...
def animate_bookend(
    ...
    # Generate 8s Veo video, then speed up to fit target duration (5s)
    veo_duration = 8
    speed_factor = veo_duration / duration  # 8/5 = 1.6x
    ...
    try:
        ...
        console.print(f"  [green]✓[/green] Veo generated {raw_path.name} ({veo_duration}s)")

        # Speed up 8s → 5s using MoviePy
        from moviepy import VideoFileClip
        clip = VideoFileClip(str(raw_path))
        sped_up = clip.with_speed_scaled(speed_factor)
        output_path.parent.mkdir(parents=True, exist_ok=True)
        sped_up.write_videofile(str(output_path), fps=30, codec='libx264', logger=None)
        sped_up.close()
        clip.close()
        raw_path.unlink(missing_ok=True)
        console.print(f"  [green]✓[/green] Sped up to {duration}s ({speed_factor:.1f}x)")
        return output_path
    except Exception as e:
...
def generate_bookends(
    ...
    for bookend_type, enabled in [("intro", generate_intro), ("outro", generate_outro)]:
        if not enabled:
            continue
