...
def combine_video_with_audio(
    ...
    # Mix original video audio (quiet) with voiceover (loud)
    # Use duration=longest so the output matches the longer track (usually voiceover)
    if has_original_audio:
        # amix: input 0 = original video audio (quiet), input 1 = voiceover (boosted)
        cmd = [
            "ffmpeg", "-y",
            "-i", str(temp_video),
            "-i", str(audio_path),
...
def create_marketing_video_veo(
    ...
    for item_idx, (item_path, item_type) in enumerate(ordered_items):
        if item_type == "video":
            ...
            try:
                ...
                console.print(f"  [dim]Original video duration: {original_vid_duration:.1f}s → target ~{int(original_vid_duration * 2.5)} words[/dim]")
            except Exception:
            ...
            # Transcribe ORIGINAL video audio first (before cleaning removes segments)
            try:
            ...
            # Strip original audio — TTS will replace it entirely
            import subprocess
    ...
    for clip in final_clips:
        if (clip.w, clip.h) != target_size:
            console.print(f"  [dim]Resizing {clip.w}x{clip.h} → {target_size[0]}x{target_size[1]}[/dim]")
            normalized_clips.append(clip.resized(target_size))
        else:
    ...
    if music_path and Path(music_path).exists():
        ...
        temp_with_music = temp_dir / "with_music.mp4"
